...
//            memcpy((uint8_t*)(q+3),p,data_len);
              *(uint8_t*)&q[0] = 0x03;
              *(uint8_t*)&q[1] = ((data_len)&0xff00)>>8;
              *(uint8_t*)&q[2] = (data_len)&0xff;
              memcpy((uint8_t*)(&q[3]),p,data_len);
            //发布消息
           ret = MQTTMsgPublish(MQTT_Socket,(char*)"$dp",QOS0,(uint8_t*)q,data_len+3);
//           if(ret >= 0)
//           {
//               printf("表明有数据交换\n");
//               //表明有数据交换
...
//               curtick = xTaskGetTickCount();				
//           }
//            ret = MQTTMsgPublish(MQTT_Socket,(char*)TOPIC,QOS0,(uint8_t*)q,data_len+3);
            if(ret >= 0)
            {
              printf("表明有数据交换\n");
                //表明有数据交换
